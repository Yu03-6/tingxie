...
function textbook() {
  ...
  layout(`<div class="crumb"><a href="#home">首页</a> / 教材</div><div class="card"><h2>选择教材内容</h2><p class="muted">按手中教材的版次和目录选择。版本在成都的具体学校使用情况仍需以学校为准。</p><div class="form-grid"><div class="field"><label for="subject">学科</label><select class="select" id="subject">${Object.entries(CATALOG).map(([key,s]) => `<option value="${key}" ${state.subject===key?'selected':''}>${s.name}</option>`).join('')}</select></div><div class="field"><label for="version">教材版本</label><select class="select" id="version">${sub.versions.map(x=>`<option value="${x.id}" ${v.id===x.id?'selected':''}>${x.name}</option>`).join('')}</select></div><div class="field"><label for="grade">年级与册次</label><select class="select" id="grade" ${g?'':'disabled'}>${g?v.grades.map(x=>`<option value="${x.id}" ${g.id===x.id?'selected':''}>${x.name} · ${x.edition}</option>`).join(''):'<option>尚未完成词表核对</option>'}</select></div></div>${g?`<p class="muted">${esc(g.name)} · ${esc(g.edition)} · ${count} 个条目 · ${g.counts.recorded ?? count} 项预录音频，其余实时合成</p><div class="unit-grid">${g.units.map(u=>`<button class="unit" onclick="chooseUnit('${u.id}')"><strong>${esc(u.name)}</strong><small>${u.items.length} 个条目　→</small></button>`).join('')}</div>${g.excludedUnits?.length?`<p class="muted">${g.excludedUnits.map(x=>esc(x.name)).join('、')}：原书附表未列听写条目。</p>`:''}<details class="source-note"><summary>教材来源与核对范围</summary><p>来源：<a href="${esc(g.source.url)}" target="_blank" rel="noopener">${esc(g.name)} PDF</a>。词表按附录和目录整理，附表 PDF 页码与课文教材页码分别标注。预录和实时语音均为合成朗读，并非出版社配套录音。</p><p>${g.language==='zh-CN'?`词语 ${g.counts.words} 项；写字 ${g.counts.writing} 项；新识字 ${g.counts.recognitionNew} 项；复习 ${g.counts.recognition-g.counts.recognitionNew} 项。${g.counts.recognition?'':'本册未收录独立识字表。'}`:'按本册词汇表整理单词和短语，分组和页码以此版本原书为准。'}</p></details>`:`<div class="empty">${esc(v.note)}。此版本尚无可用单元。</div>`}</div>`);
...
function useCustom(){
  ...
  state.customText=text;state.customLang=document.querySelector('#customLang').value;state.voiceMode=document.querySelector('#voiceMode').value;try{localStorage.setItem('tingxie-custom',text);}catch{}
  state.items=items.map(text=>{const language=state.customLang==='auto'?(/[\u3400-\u9fff]/u.test(text)?'zh-CN':'en-US'):state.customLang;return {text,language,voiceMode:state.voiceMode,audio:audioIndex.get(`${language}:${text.toLowerCase()}`)};});
  state.origin='custom';go('settings');
}
function settings(){
  if(!state.items.length)return custom();const audioCount=state.items.filter(x=>x.audio).length;
  layout(`<div class="crumb"><a href="#${state.origin==='custom'?'custom':'content'}">返回内容</a> / 听写设置</div><div class="card"><h2>定制听写节奏</h2><p class="muted">${state.items.length} 项内容，${audioCount} 项已配音。${audioCount<state.items.length?'其他词使用系统语音或本地兼容语音，首次准备可能稍慢。':''}</p><form id="settingsForm"><div class="field" style="margin-bottom:20px"><label for="sessionVoice">未配音内容的播放方式</label><select class="select" id="sessionVoice"><option value="auto">自动选择</option><option value="local">兼容语音（无声时选择，音色较机械）</option></select></div><div class="settings">${[['repeat','每项重复次数',1,10,1],['repeatGap','重复间隔（秒）',0,60,.5],['itemGap','换词间隔（秒）',0,120,.5],['rounds','总循环次数',1,20,1],['rate','朗读速度',.5,1.5,.1]].map(([key,label,min,max,step])=>`<div class="setting"><label for="${key}">${label}</label><input class="input number" id="${key}" type="number" required min="${min}" max="${max}" step="${step}" value="${state[key]}"></div>`).join('')}</div><div class="actions" style="margin-top:24px"><button class="btn" type="submit">准备开始听写　→</button></div></form></div>`);
  document.querySelector('#sessionVoice').value=state.voiceMode;
  document.querySelector('#settingsForm').onsubmit=e=>{e.preventDefault();state.voiceMode=document.querySelector('#sessionVoice').value;state.items=state.items.map(x=>({...x,voiceMode:state.voiceMode}));for(const key of ['repeat','repeatGap','itemGap','rounds','rate'])state[key]=Number(document.getElementById(key).value);engine.stop();state.hidden=false;go('player');};
}
function startSession(){engine.transport.unlock();engine.start(state.items,{repeat:state.repeat,repeatGap:state.repeatGap,itemGap:state.itemGap,rounds:state.rounds,rate:state.rate});}
function player(){
  if(!state.items.length)return custom();const s=engine.snapshot();const item=state.items[s.index]||state.items[0];
  const focusedWord=document.activeElement?.dataset.wordIndex;
  const listScroll=document.querySelector('.word-list')?.scrollTop || 0;
  const active=['speaking','waiting'].includes(s.status);const finished=s.status==='completed';
  const canSeek=['speaking','waiting','paused','error','completed'].includes(s.status);
  const positionLabel=s.status==='waiting'||(s.status==='paused'&&engine.pausedFrom==='waiting')?'即将朗读':s.status==='paused'?'已暂停':s.status==='error'?'播放失败':'当前词语';
  const words=`<section class="session-words" aria-labelledby="word-list-title"><h2 id="word-list-title">本次听写词语</h2><p class="muted" id="word-list-help">${canSeek?'没听清？点击任意一项，从该词重新朗读并继续听写。':'开始听写后，可点击任意一项跳转续听。'}${state.hidden?'当前已隐藏词语，可按序号选择。':''}</p><ol class="word-list" aria-describedby="word-list-help">${state.items.map((word,index)=>`<li><button type="button" class="word-jump" data-word-index="${index}" ${canSeek?'':'disabled'} ${canSeek&&!finished&&index===s.index?'aria-current="step"':''} onclick="engine.transport.unlock();engine.seek(${index})"><span class="word-number">${index+1}</span><span class="word-text">${state.hidden?`第 ${index+1} 项`:esc(word.text)}</span>${canSeek&&!finished&&index===s.index?`<span class="word-position">${positionLabel}</span>`:''}</button></li>`).join('')}</ol></section>`;
  ...
  document.querySelector('.word-list').scrollTop=listScroll;
  if(focusedWord!==undefined)document.querySelector(`[data-word-index="${focusedWord}"]`)?.focus({preventScroll:true});
}
